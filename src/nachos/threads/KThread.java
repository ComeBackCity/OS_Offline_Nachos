...
public class KThread {
	...
	Machine.interrupt().disable();
		...
		ThreadQueue cWaitingQueue = currentThread.waitingQueue;
		if(cWaitingQueue != null) {
			KThread thread = cWaitingQueue.nextThread();
			if (thread != null) {
				thread.ready();
			}
		}
	...
	public static void test(){
		...
		new KThread(new PingTest(1)).setName("Thread 0").fork();
		new KThread(new PingTest(2)).setName("Thread 0").fork();
		//new PingTest(3).run();

		KThread t1 = new KThread(new PingTest(1)).setName("Thread 0");
		KThread t2 = new KThread(new PingTest(2)).setName("Thread 0");
	...
	new PingTest(0).run();
		System.out.println("In self test for thread");*/
		//test();
    }
